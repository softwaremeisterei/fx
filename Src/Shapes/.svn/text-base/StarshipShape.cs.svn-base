...
namespace fx
{
	...
	public class StarshipShape : Shape
	{
		...
		public override void Paint(System.Drawing.Graphics g, double x, double y, double scale) 
		{
			...
			turtle.Save();
			turtle.Left(CosmicConstants.Degree90);
			turtle.Forward(this.Width/2 * scale);
			turtle.Right(Math.Atan(this.Width/2/this.Height) + CosmicConstants.Degree90);
			turtle.Forward(Math.Sqrt(this.Width * this.Width / 4 + this.Height * this.Height) * scale);

			turtle.Recall();
			turtle.Right(CosmicConstants.Degree90);
			turtle.Forward(this.Width/2  * scale);
			turtle.Left(Math.Atan(this.Width/2/this.Height) + CosmicConstants.Degree90);
			turtle.Forward(Math.Sqrt(this.Width * this.Width / 4 + this.Height * this.Height) * scale);
		}
	}
}
